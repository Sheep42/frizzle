...
class( "Playing_CopyGame_Phase2_Glitch" ).extends( Microgame )
local scene = Playing_CopyGame_Phase2_Glitch
...
function scene:update()
	...
	if self.timer.value >= 3000 or self.win then
		...
		if self.timer.value >= ONE_SECOND * 5 then
			...
			Noble.transition( LivingRoomScene, 0.1, Noble.TransitionType.CROSS_DISSOLVE )
			return
		end
